...
if (getParameter("cancel")) {
  ...
  if ($insert) {
    ...
    $results = mysqli_query($_LINK, "SELECT *" .
      " FROM DATASET" .
    ...
    if (mysqli_num_rows($_LINK, $results) != 0) {
      $errors["DATA_SET_ID"] = "Dataset ID already in database.";
    }
  }
  ...
  if (count($errors) > 0) {
    ...
    if ($insert) {
      ...
      if ($uuid == 0) {
        ...
        mysqli_query($_LINK, "UPDATE DATASET" .
          " SET CREATE_DTS = now()" .
          " WHERE UUID = " . ($uuid + 0));
      ...
      mysqli_query($_LINK, "UPDATE DATASET SET" .
        join(",", $updateTerms) .
        " WHERE UUID = $uuid");
